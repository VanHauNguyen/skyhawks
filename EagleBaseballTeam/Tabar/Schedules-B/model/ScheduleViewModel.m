...
{
    WS(weakSelf);
    NSString *authString = [NSString stringWithFormat:@"%@:%@", [[EGCredentialManager sharedManager] getUsername], [[EGCredentialManager sharedManager] getPassword]];    NSData *authData = [authString dataUsingEncoding:NSUTF8StringEncoding];
...
//    [MBProgressHUD showMessage:@""];
    [[WAFNWHTTPSTool sharedManager] getWithURL:url parameters:@{@"year":year,@"kindCode":@"A",@"teamNo":@"AKP011"} headers:dict_header success:^(id  _Nonnull response) {
        ...
        for (EGScheduleModel *model in modelArr) {
            if ([model.HomeTeamCode isEqualToString:@"AKP011"] || [model.VisitingTeamCode isEqualToString:@"AKP011"]) {
