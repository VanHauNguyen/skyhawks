...
-(void)setSchedulesInformation:(EGScheduleModel *)model
{
    ...
    self.rightImageView.image = [UIImage imageNamed:retureTeamIconImgName(model.HomeTeamCode)];
    self.leftImageView.image = [UIImage imageNamed:retureTeamIconImgName(model.VisitingTeamCode)];
    
    if (model.GameResult.length < 1) {
...
//例行賽 307
        ...
        if ([model.HomeTeamCode isEqualToString:@"AKP011"]) {//雄鹰主场才能买票
        ...
        self.titleLb.text = [NSString stringWithFormat:@"例行賽  %ld",model.GameSno];//@"例行賽 307";
...
//例行賽 307
        ...
        if ([model.GameResult isEqualToString:@"0"]) {//有比分
            
            self.rightScoreLb.text = [NSString stringWithFormat:@"%ld",model.HomeScore];
            self.leftScoreLb.text = [NSString stringWithFormat:@"%ld",model.VisitingScore];
...
//            [self.leftBtn setTitle:@"鷹迷回顧" forState:UIControlStateNormal];
        ...
        self.titleLb.text = [NSString stringWithFormat:@"例行賽 %ld｜%@",model.GameSno, model.FieldAbbe];
        self.titleLb.textColor = rgba(115, 115, 115, 1);
        
        
        if (![model.GameResult isEqualToString:@"0"]) { //0正常結束 1延赛 2保留
            
            if ([model.GameResult isEqualToString:@"1"]) {
...
{
    if ([homeTeamCode containsString:@"AAA011"]) {
        return @"味全龍3x_L";
    }else if ([homeTeamCode containsString:@"ADD011"]){
        return @"統一獅3x_L";
    }else if ([homeTeamCode containsString:@"AEO011"]){
        return @"富邦悍將3x_L";
    }else if ([homeTeamCode containsString:@"AKP011"]){
        return @"台鋼雄鷹3x_L";
