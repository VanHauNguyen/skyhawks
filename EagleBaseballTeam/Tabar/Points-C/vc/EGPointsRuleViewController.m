...
//        make.bottom.mas_equalTo(scrollView);
    ...
    NSArray *Array =@[@{@"title":@"點數辦法：",
                        @"subtitles":@[],
                        @"contents":@[@"雄鷹點數為台鋼雄鷹 APP (以下簡稱本程式)內提供之獎勵點數，鼓勵消費者免費下載本程式註冊使用，在依照本程式內說明完成相關任務指定條件後，即可累積點數兌換限量贈品、活動報名或抽獎券。 "],
                        @"numContents":@[
                      ...
                      @{@"title":@"任務說明：",
                        @"contents":@[@"一、本程式內提供之各式點數任務，請參考任務內說明文字進行。每個任務同一會員帳號及行動裝置每日僅計算一次(消費型任務除外)，恕不接受申請追溯或補登次數。任務僅限會員本人完成。\n\n二、台鋼雄鷹主場例行賽事相關之每日任務，在賽事結束後依照實際賽況給予不同點數獎勵。各任務均為獨立事件，易可同時達成(如和局與延長賽)。依類別區分說明如下： "],
                        @"subtitles":@[@"會員專屬任務",@"每日任務",@"限時任務"],
                        @"numContents":@[
                            @"需先申辦付費會員，即可增加會員專屬任務，每一會員帳號不限申辦一個會員資格，詳細內容可查看任務說明。 ",
                            @[@"須購買台鋼雄鷹主場內野門票並至球場內野指定感應區，透過個人行動裝置(需開啟藍芽、定位與APP權限)開啟本程式，即可完成每日任務｢鷹雄軍報到｣。系統將自動觸發指定日期進場任務(空投給點)及累積成就勳章進場次數。",
                             @"同一帳號及裝置每場限計算一次，如同一裝置以多帳號登入，僅限當日第一個帳號可感應觸發。場次紀錄恕不接受事後補登，如現場未能完成感應，請於當日離場前置球迷服務台進行確認，離場後恕無法提供補登服務。",
                            @"｢比賽結果(如:勝敗和)｣、｢賽事內容(如雄鷹無失誤等)｣類須先完成每日任務｢鷹雄軍報到｣後，系統將於當天23:59前依據比賽內容、結果、日期等條件，自動進行計算累積給點(非賽後即時)。未於當日完成該場賽事｢每日任務-鷹雄軍報到｣者，將無法成功觸發本任務。 ",
                            @"當日於台鋼雄鷹球場實體店面消費，結帳前須出示本程式會員條碼，系統將於隔日自動累積消費金額，若達成任務指定消費，將給予對應點數。 "],
                            @[@"不定時發布限時任務，依照任務說明完成後將另以人工方式空投給點、掃碼給點或即時給點(特殊任務將另以任務內文字說明為準)，請隨時關注台鋼雄鷹APP或開啟推播通知已獲得額外點數回饋。",
                            @"於台鋼雄鷹實體店面消費，結帳前須出示本程式會員條碼，系統將於隔日自動累積年度消費金額，若達成任務指定消費，將給予對應點數。 "]
                        ]},
                      @{@"title":@"其他說明：",
                        @"contents":@[@""],
                        @"subtitles":@[@""],
                        @"numContents":@[
                            @"凡持中華職棒大聯盟或台鋼雄鷹球團核發證件之球場從業人員及其親屬、使用無價票券及台鋼雄鷹全體員工，均不得兌換、參加本點數相關贈品及活動。 ",
                            @"凡參與積點活動之會員，須保證過程中所填寫或提出之資料均正確無誤。若以大量創建帳號、冒用或盜用他人資料、資料不正確、使用惡意電腦程式或其他任何違反點數任務說明及影響公平性之方式完成任務、進行點數蒐集或贈品兌換者，台鋼雄鷹球團得凍結該會員帳號及取消其所獲得之點數與使用權利。 ",
//                            @"如有點數相關疑問，請洽現場球迷服務台工作人員或透過APP內聯絡客服功能詢問。 ",
                            ...
                            @"台鋼雄鷹保留隨時增訂、修改、解釋與中指本辦法之權利，並以官方公告為準，不另個別通知",
