...
-(void)getclassinfo
{
    ...
    [[WAFNWHTTPSTool sharedManager] getWithURL:[EGServerAPI get_FansClass] parameters:@{} headers:headerDict success:^(NSDictionary *  _Nonnull response) {
        ...
        if(array.count>0)
        {
            ...
            [self mergeArray:array girlNO:[self.girlDetailinfo objectForKey:@"girlNO"]];
        }
        else
    ...
    for(int i=0;i<arr.count;i++)
    {
        ...
        for(int y=0;y<date_girl.count;y++)
        {
            ...
            NSPredicate *predicate1 = [NSPredicate predicateWithBlock:^BOOL(id evaluatedObject, NSDictionary *bindings) {
                NSString *dateV = (NSString *)evaluatedObject[@"ws_number"];
...
//                NSString* girl_number = [dateinfo objectAtIndex:0][@"ws_number"];
//                if([girl_number isEqualToString:girl_No])
//                 [dic setObject:[NSNumber numberWithBool:YES] forKey:@"girlclassdatestatus"];
//                else
//                [dic setObject:[NSNumber numberWithBool:NO] forKey:@"girlclassdatestatus"];
...
//    NSLog(@"year = %ld, month = %ld",self.current_year,self.current_month);
    ...
    if(dateinfo.count>0){
        [self has_classData:YES];
        [self mergeArray:[dateinfo objectAtIndex:0][@"ws_month_data"] girlNO:[self.girlDetailinfo objectForKey:@"girlNO"]];
    }
    else
        [self has_classData:NO];
}
